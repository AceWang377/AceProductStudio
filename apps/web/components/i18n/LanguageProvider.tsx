...
const phrasePairs = [
  ...
  ["Growth Studio", "增长工作台"],
  ["Shopify audit ready", "Shopify 审核已就绪"],
  ["Connect Shopify for live audit", "连接 Shopify 进行实时审核"],
  [
    "Score Shopify products for SEO and GEO before you approve updates.",
    "在确认更新前，先为 Shopify 商品进行 SEO 和 GEO 评分。"
  ],
  [
    ...
    "读取已连接 Shopify 店铺的商品，找出标题、描述、图片语义和 AI 问答缺口，并且只在用户确认后写入所选优化。"
  ],
  ["Open product workflow", "打开商品流程"],
  ["SEO/GEO guide", "SEO/GEO 指南"],
  ["Audit source", "审核来源"],
  ["Live Shopify products", "实时 Shopify 商品"],
  ["AceStudio workspace", "AceStudio 工作区"],
  ["Store health", "店铺健康度"],
  ["SEO/GEO audit summary", "SEO/GEO 审核总结"],
  [
    "This MVP scores products, creates fix recommendations, and keeps Shopify write-back behind a per-product confirmation step.",
    "这个 MVP 会给商品评分、生成优化建议，并且每个商品都需要单独确认后才会写回 Shopify。"
  ],
  ["Score first", "先评分"],
  ["SEO checks", "SEO 检查"],
  ["GEO checks", "GEO 检查"],
  ...
  [
    ...
    "Growth Studio 会先审核。SEO 标题、Meta 描述、标签和追加的买家问答只有在用户确认后才会写入 Shopify。"
  ],
  ["Product scores", "商品评分"],
  ["Lowest scoring products first", "低分商品优先"],
  ["Open products", "打开商品"],
  ["Top recommendations", "重点优化建议"],
  ["Suggested update", "建议更新"],
  ["Apply SEO/GEO fixes", "应用 SEO/GEO 优化"],
  ["Confirm apply", "确认应用"],
  ["Cancel", "取消"],
  ["Applied", "已应用"],
  ["Applying...", "正在应用..."],
  ["Open in Shopify", "在 Shopify 中打开"],
  ["Open draft", "打开草稿"],
  ["Credits", "积分"],
  ["Sign in", "登录"],
  ["Sign out", "退出"],
  ...
  ["AI Shopify product listings, reviewed before they publish.", "AI Shopify 商品页，发布前可先审核。"],
  ["Product screenshot", "产品截图"],
  ["See the full listing before Shopify receives it.", "在发送到 Shopify 前先看到完整商品页。"],
  [
    ...
    "AceStudio 会把每个生成商品变成可审核界面：图片顺序、SEO 文案、价格库存、发布状态和 Shopify 草稿链接都清楚可见。"
  ],
  ["generated images", "生成图片"],
  ["draft-first publish flow", "默认草稿发布流程"],
  ["manual token paste for OAuth users", "OAuth 用户无需手动粘贴 token"],
  ["Product review", "商品审核"],
  ["Generated product set", "生成商品组"],
  ["Draft safe", "草稿安全"],
  ["Publish checklist", "发布检查清单"],
  ["Media order approved", "图片顺序已确认"],
  ["SEO copy reviewed", "SEO 文案已检查"],
  ["Price and inventory set", "价格和库存已设置"],
  ["Shopify draft link saved", "Shopify 草稿链接已保存"],
  ["Next action", "下一步操作"],
  ["Draft-first", "默认草稿"],
  ...
  ["Replace scattered product setup with one reviewable workflow.", "用一个可审核流程替代分散的商品配置。"],
  [
  ...
  ["Image workflow", "图片流程"],
  ["Separate tabs and downloads", "分散的标签页和下载"],
  ["Labeled media set in one product", "同一商品内有标签的图片组"],
  ["Listing copy", "商品文案"],
  ["Manual copy paste", "手动复制粘贴"],
  ["Editable SEO title, tags, FAQ, and description", "可编辑 SEO 标题、标签、FAQ 和描述"],
  ["Publish safety", "发布安全"],
  ["Easy to push live too early", "容易过早正式上线"],
  ["Draft-first Shopify publishing", "默认 Shopify 草稿发布"],
  ["Recovery", "恢复处理"],
  ["Errors handled by screenshots", "靠截图处理错误"],
  ["Saved jobs, retry paths, and Shopify links", "保存任务、重试路径和 Shopify 链接"],
  ...
  ["What does one credit pay for?", "一个积分用于什么？"],
  [
    "Credits are designed around usage events such as image generation and future paid workflows. Stripe can be enabled later without changing the listing workflow.",
    "积分会围绕图片生成等使用事件设计，后续接入 Stripe 时无需改变商品页流程。"
  ],
  ["Can merchants edit the generated listing?", "商家可以编辑生成的商品页吗？"],
  [
    "Yes. Copy, media review, price, SKU, and inventory fields stay editable before the product is sent to Shopify as a draft.",
    "可以。文案、图片审核、价格、SKU 和库存字段在发送 Shopify 草稿前都可以编辑。"
  ],
  ["Is this only for one Shopify store?", "只能用于一个 Shopify 店铺吗？"],
  [
    "No. The app is built around per-user Shopify OAuth connections, so different users can connect different stores.",
    "不是。应用基于每个用户的 Shopify OAuth 连接，不同用户可以连接不同店铺。"
  ],
  ["Upload one product photo and build the first Shopify draft.", "上传一张商品图，创建第一个 Shopify 草稿。"],
  ["Start with draft publishing, then add paid credits when Stripe is ready.", "先从草稿发布开始，Stripe 准备好后再加入付费积分。"],
  ["Open studio", "打开工作室"],
  ["View product workflow", "查看商品流程"],
